...
public class GameManager : MonoBehaviour
{
    ...
    public void Matched()       //카드 두장이 뒤집혔을 때 실행
    {
        if (firstCard.idx == secondCard.idx )  //카드 두장이 동일
        {
            ...
            switch (secondCard.idx) {            //  idx 숫자마다 text의 내용 변경    
                case 0:
                case 1:
                    ResultTxt.text = "김희환";
                    break;
    ...
    public void GameEnd() //한 스테이지 성공했을때 
    {
        ...
        int cardNumber = 0; ; // 총카드수
        flipCountTxt.text = flipCount.ToString(); //뒤집은 횟수 
        if (PlayerPrefs.GetInt("level") == 0)   //노말난이도
        {
            defaultScore = 30;
            givenTime = 60.0f;
            cardNumber = 16;
        ...
        finalScoreTxt.text = ((defaultScore + (int)(givenTime - time) + (cardcount-cardNumber))*2 - (20-flipCount)).ToString();
    ...
    {
        if(finalScoreTxt.text != "0") { //최종점수가 0이아니라면 실행 ,스테이지 못 깨면 최종점수가 0이기 때문이다
